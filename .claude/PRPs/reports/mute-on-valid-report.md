...
| # | 実測 | 実装の誤り | フィクスチャの誤り |
|---|---|---|---|
| 1 | click 直後の `[role="menu"]` が **0 件**、300ms 後に 1 件 | **押した直後に同期で読んでいた。**React は状態更新を同期で描画しない | **メニューを最初から DOM に置いていた。**「押した瞬間はまだ無い」経路が一度も走らない |
| 2 | `"volume_off投稿ユーザーをミュート"` | **完全一致にしていた。**Material Symbols のアイコンは**テキスト**として前に付く | **項目のテキストをラベルだけにしていた。**アイコンが混ざることに気づけない |
