 ...
 * 全員持っていないと計算できない。**分母が小さいだけで 0 でないのが最も危険**で、
 * 3/3 = 100% と自信満々に出してしまう。欠け方は 2 通りあり、どちらも
 * `itemCoveredMinutes`（投稿から何分後までを全部持っているか）に集約されている。
 *
 *   件数で欠ける: `page=1` は降順で「最も新しい 100 件」なので、100 いいねを
 *     超える記事では窓内が丸ごと落ちる（2026-08-25 実測）。末尾から遡って解決
 *   時間で欠ける: 記事は 1 度しか取らないのに、窓は投稿から先へ伸び続ける。
 *     **投稿 20 分後に取った記事は、その時点の全部を持っていても 180 分の窓を
 *     覆えていない**（2026-08-30 実測）。取り直さない限り永久に埋まらない
 *
 * 後者は「全部持っているか」では判定できない。**1 記事でも窓を覆えていなければ、
 * 測定そのものを放棄する。**
 *
...
export function windowShare(
  ...
  // **1 記事でも窓を覆えていなければ測らない。**部分的な分母は過大な占有率を出す
  for (const itemId of shared) {
    // 覆った範囲が分かっているなら、**それだけで決まる。**
    //
    // 見るのは「全部持っているか」ではなく「**窓を覆っているか**」。投稿 20 分後に
    // 取った記事は、その時点の全部を持っていても 180 分の窓を覆えていない
    // （2026-08-30 実測: 窓を 60 分から 2 日まで動かしても同じ 3/5 を返した）。
    // 逆に 100 件超の記事は末尾から遡るので「真ん中が欠けたまま 0〜N 分は完全」に
    // なるが、その形でも窓さえ覆っていれば分母は揃う（scanner の collectLikes）
    const reach = covered.get(itemId);
    if (reach !== undefined) {
      if (windowMinutes <= reach) continue;
      return null;
    }

    // ここから下は itemCoveredMinutes を持たない**古いレコード専用**の経路。
    // **いつ取ったかが残っていないので、窓を覆えたかは判定できない。**
    // せめて「取得時点で取りこぼしていないか」だけを見る。
    //
    // 記事は再取得しない（scanner の known / seen）ので、ここで救わないと
    // **この機能より前に蓄積した候補は永久に「測れません」のまま**になる。
    ...
    // Total-Count も記録していないレコード（この機能より前に取ったもの）。
    // **per_page が上限なので、それ未満しか無ければ「それが取得時点の全部」。**
