...
describe('badgeText', () => {
  ...
  // 「4 文字を超えない」は **検査していない**。badgeText は件数を丸めておらず、
  // 5 桁を渡せば 5 文字返る。以前ここに置いていた 9999 のテストは、たまたま
  // 4 桁だから通っていただけで、実装が守っていない性質を守っているように
  // 見せていた。件数の上限はトレンドの表示件数（実測 25〜30）で決まる
});
